    ...
    <nav className="relative z-50 bg-white shadow px-6 py-4">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <div className="flex items-center space-x-8">
          <Link to="/" className="text-2xl font-bold text-green-900">HealthCare</Link>
          <ul className="hidden lg:flex items-center space-x-6">
            ...
            <li><Link to="/dashboard/vr" className="hover:text-green-600">Virtual Robot</Link></li>
        ...
        <div className="flex items-center space-x-4">
          ...
          {isAuth ? (
            <button onClick={handleLogout} className="px-4 py-2 bg-red-100 text-red-700 rounded-full hover:bg-red-200">
              Logout
            </button>
          ) : (
            <Link to="/login" className="px-4 py-2 bg-green-100 text-green-700 rounded-full hover:bg-green-200">
              Login
            </Link>
          )}
          <button
      ...
      {mobileOpen && (
        <div className="lg:hidden bg-white border-t border-gray-200 shadow-md">
          <ul className="space-y-1 px-4 py-3">
            <li><Link to="/" onClick={() => setMobileOpen(false)} className="block px-2 py-2 rounded hover:bg-green-50">Home</Link></li>
            <li>
              <button
                onClick={() => setServicesOpen(o => !o)}
            ...
            <li><Link to="/dashboard/vr" onClick={() => setMobileOpen(false)} className="block px-2 py-2 rounded hover:bg-green-50">Virtual Robot</Link></li>
            <li><Link to="/contact" onClick={() => setMobileOpen(false)} className="block px-2 py-2 rounded hover:bg-green-50">Contact</Link></li>
            <li>
              {isAuth
                ? <button onClick={handleLogout} className="w-full text-left px-2 py-2 rounded text-red-600 hover:bg-red-50">Logout</button>
                : <Link to="/login" onClick={() => setMobileOpen(false)} className="block px-2 py-2 rounded text-green-600 hover:bg-green-50">Login</Link>
              }
            </li>
          </ul>
        </div>
